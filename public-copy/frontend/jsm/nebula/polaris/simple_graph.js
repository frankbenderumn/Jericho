...
class Graph {
    constructor(width, height, wrapper_id, parameter = "line", ticker = "undefined", frame = 1) {
        this.width = width;
        this.height = height;
        this.size = 25;
        ...
        document.getElementById(wrapper_id).style.float = "left";
        ...
        document.getElementById(wrapper_id).append(this.canvas);
        ...
        this.xLabel = "none";
        this.yLabel = "none";
        this._title = "Accuracy";
        this._xTitle = "Fusions";
        this._yTitle = "Accuracy";
        this.weightA = 75;
        this.weightB = 25;
        this.xLabel = "Time (ET)";
        this.yLabel = "Unknown";
        ...
        // placeholders
        for (let i = 0; i < (this.linesY - this.yAxis); i++) {
            this.labels_.push(i);
            this._labels.push(i);
            this.points.push(i);
        }
        this.yLabel = "Accuracy";
        this.pointsNorm = this._normalize(this.points);
    ...
    computeDims() {
        ...
        for (let i = this.xBegin; i < this.xEnd; i++) {
            let yVal = -(ys[i] * (this.yBins * (ys.length)) + this.lineOffset);
    ...
    intersect() {
        let result = 0;
        let xLock = 0;
        if (debug) console.log("XBIN LENGTH IS: " + this.xBins);
        if (debug) console.log("cursor.x: " + this.cursor.x);
    ...
    drawLineIntersect(xVal) {
        ...
        xVal += this.size;
        this.ctx.beginPath();
        if (debug) console.log("DRAWING CURSOR CIRCLE AT: <" + xVal + ", " + yVal + ">");
    ...
    drawBar(points) {
        ...
        for (let i = this.xBegin; i < this.xEnd; i++) {
            let lastY = 0;
            if (i > this.xBegin) {
                lastY = -(ys[i - 1] * this.yBins * (this.points.length) + this.lineOffset);
            }
            let yVal = -(ys[i] * this.yBins * (this.points.length) + this.lineOffset);

            if (yVal <= lastY) {
                this.ctx.fillStyle = "#7aff7c";
            } else {
                this.ctx.fillStyle = "#ff7a7a";
            }
            // this.ctx.lineTo((i - this.xBegin) * this.xBins, yVal);
            // this.fillGraph.push({x: (i - this.xBegin) * this.xBins, y: yVal});
    ...
    drawLine(points) {
        ...
        this.ctx.moveTo(0, 0);
        if (ys.length > 0) {
            let yInit = -(ys[0] * (this.yBins * (ys.length)) + this.lineOffset);
            this.ctx.moveTo((0 - this.xBegin) * this.xBins, yInit);
        }
        this.fillGraph = [];
        ...
        this.fillGraph.push({x: this.width, y: 0});
    ...
    labels() {
        ...
        for(let i = 1; i < (this.linesY - this.yAxis); i += this.xLabelInterval) {
            this.ctx.beginPath();
            this.ctx.lineWidth = 1;
        ...
        let yLabelCt = (this.linesX - this.xAxis);
        ...
        let interval = parseFloat(this.yRangeEnd - this.yRangeStart) / parseFloat(this.xAxis);
        ...
        for(let i = 1; i < this.xAxis; i++) {
            ...
            this.ctx.font = '9px Arial';
            this.ctx.textAlign = 'start';
            this.ctx.fillText((interval * i).toFixed(1) + this.yStart.suffix, -20, -this.size*i+3);
        }
    }
    ...
    update(xs, ys) {
        ...
        this.rawDataX = xs;
        this.rawDataY = copy;
        console.log(this.rawDataY);
        this.pointsNorm = this._normalize(ys);
        console.log(this.rawDataY);
    }
    ...
    append(y) {
        let yCopy = y.map((x) => x);
        this.points = yCopy;
        let max = this.max(yCopy);
        let min = this.min(yCopy);
        let range = max - min;
        let currentRange = this.yRangeEnd - this.yRangeStart;
        if (range > (1 + 0.2) * currentRange) {
            this.yRangeEnd = max + (0.1 * currentRange);
            this.yRangeStart = min - (0.1 * currentRange);
        }
        this._autoLabels.push(++this._autoLabel);
        let copy = this.points.map((x) => x);
        this._labels = this._autoLabels;
        this.points = this.points;
        this.rawDataX = this._autoLabels;
        this.rawDataY = copy;
    }
    ...
    drawHud() {
        ...
        this.ctx.font = '12px Arial';
        this.ctx.textAlign = 'start';
        this.ctx.fillText(this.ticker.toUpperCase(), this.width - 80, 20);
    ...
    rangeX(start, end) {
        this.xRangeStart = start;
        this.xRangeEnd = end;
    }

    rangeY(start, end) {
        this.yRangeStart = start;
        this.yRangeEnd = end;
    }

    scaleX() {
        let copy = this.points.map((x) => x);
        let results = this._rescale(copy, this.xRangeStart, this.xRangeEnd);
        return results;
    }

    scaleY() {
        let copy = this.points.map((x) => x);
        let results = this._rescale(copy, this.yRangeStart, this.yRangeEnd);
        return results;
    }
    ...
    _normalize(input) {
        let result = input.map((x) => x);
        let max = this.max(input);
        let min = this.min(input);
        let pad = 0.1 * (max - min);
        for (let i = 0; i < input.length; i++) {
            result[i] = ((input[i] - min + pad) / (max - min)).toFixed(5);
        }
        return result;
    }

    _rescale(input, min, max) {
        let result = input.map((x) => x);
        for (let i = 0; i < input.length; i++) {

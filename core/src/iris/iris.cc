...
namespace iris {
    ...
    std::string interpret(Router* router, std::string file, bool path) {
        ...
        LEXES forms = scanzz(file_contents, std::regex("\\$\\[iform(=.+)?\\]\\{[\\s\\S]+\\}\\$\\r\\n"));
        ...
        if (ls.size() > 0) {
            ...
            BRED("Iris::interpret: idoc not detected\n");
        }
        ...
        if (links.size() > 0) {
            ...
            BRED("Iris::interpret: ilink not detected\n");
        }
        ...
        if (forms.size() != 0) {
            ...
            for (auto lex : forms) {
                ...
                std::string newForm = parseForm(lex);
                replace(file_contents, lex.first, newForm);
            }
        } else {
            BRED("Iris::interpret: iform not detected!\n");
        }
        ...
        if (temps.size() != 0) {
            ...
            BRED("Iris::interpret: itemp not detected!\n");
        }
        ...
        if (partials.size() > 0) {
            ...
            BRED("Iris::interpret: ipart not detected!\n");
        }
    ...
    std::string parseLink(Router* router, LEX lex) {
        ...
        if (linkPath != "") {
            if (router->contains(linkPath)) {
                std::string ipath = router->ipath(linkPath);
                if (label == "") {
                    label = ipath;
                }
